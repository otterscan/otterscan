    ...
    <ContentFrame tabs>
      <SelectionContext.Provider value={selectionCtx}>
        <BlockNumberContext.Provider value="latest">
          <div className="my-2 p-3 bg-yellow-50">💡 We consider INFINITE anything &gt;= {ALLOWANCE_MIN_INFINITE.toString()}</div>
          <div className="grid grid-cols-12 gap-x-1 bg-gray-100 border-t border-b border-gray-200 px-2 py-2 font-bold text-gray-500 text-sm mt-3">
            <div className="col-span-3">Spender</div>
            <div className="col-span-4">Allowance</div>
          ...
          {approvals ? (
            <>
              {approvals.map((a) => (
                ...
                >
                  <span className="col-span-3 flex items-baseline">
                  ...
                  <span className="col-span-4 flex items-baseline">
                    <TokenBalance address={address} token={a.token} />
                  </span>
                  <span className="col-span-1 flex items-baseline">Revoke</span>
                </div>
              ))}
            </>
          ) : (
