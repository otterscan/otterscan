 ...
 * INPUT GRAMMAR:
 * Start := _ values=Values _ $
 *     .value = any[] { return this.values.value; }
...
export class Parser {
  ...
  public matchStart($$dpth: number, $$cr?: ErrorTracker): Nullable<Start> {
    return this.run<Start>($$dpth, () => {
      let $scope$values: Nullable<Values>;
      let $$res: Nullable<Start> = null;
      if (
        true &&
        this.match_($$dpth + 1, $$cr) !== null &&
        ($scope$values = this.matchValues($$dpth + 1, $$cr)) !== null &&
        this.match_($$dpth + 1, $$cr) !== null &&
        this.match$EOF($$cr) !== null
      ) {
        $$res = new Start($scope$values);
      }
      return $$res;
    });
  }
